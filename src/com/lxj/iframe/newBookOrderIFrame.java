...
import com.lxj.JComPz.Item;
import com.lxj.Dao.BookInfoDao;
import com.lxj.Dao.BookTypeDao;
import com.lxj.Dao.NewBookDao;
...
public class newBookOrderIFrame extends JInternalFrame {
	...
	class ButtonAddLisenter implements ActionListener{
		public void actionPerformed(final ActionEvent e) {
			...
			if(!NewBookDao.selectBookOrder(ISBN.getText().trim()).isEmpty()){
			...
			try{
				int i=NewBookDao.InsertBookOrder(ISBN.getText().trim(), java.sql.Date.valueOf(orderDate.getText().trim()), orderNumber.getText().trim(), operator.getText().trim(), checkAndAccept,zks);
	...
	class ISBNListener extends KeyAdapter {//使用回车键进行触发事件方法
		public void keyTyped(KeyEvent e) {
			if (e.getKeyChar() == '\n') { // 判断在文本框是否输入回车。
				...
				List list =BookInfoDao.selectBookInfo(ISBNs);
